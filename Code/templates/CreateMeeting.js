...
document
  ...
  .addEventListener("click", async function () {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error("서버에서 친구 이름 데이터를 가져오지 못했습니다.");

      const data = await response.json();
      const names = data.names.filter(
        (name) => !selectedFriendNames.includes(name)
      );
...
function addFriendToSelection(name) {
  const selectedFriendsContainer = document.getElementById(
  ...
  try {
    ...
    if (meetings.length === 0) {
      ...
      meetings.forEach((meeting) => {
        ...
        // 친구 이름을 카드 형식으로 표시
        const friendsHTML = meeting.friends
          .map(
            (friend) => `
        ...
        <a href="/recommend?meeting_name=${encodeURIComponent(
          meeting.name
        )}" style="text-decoration:none; color:inherit;">
          <div class="meeting-header">
            <h3>${meeting.name}</h3>
            <button class="delete-meeting" data-id="${
              meeting.id
...
async function deleteMeeting(meetingId) {
  if (confirm("정말로 이 모임을 삭제하시겠습니까?")) {
    try {
      const response = await fetch(`/delete-meeting/${meetingId}`, {
        method: "DELETE",
      });
      const result = await response.json();
      if (result.status === "success") {
