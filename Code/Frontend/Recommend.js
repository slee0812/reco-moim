...
if (meetingName && meetingDate && meetingTime) {
  ...
  if (friendsContainer) {
    const friendsArray = invitedFriends ? invitedFriends.split(',').map(friend => friend.trim()) : [];
    friendsContainer.innerHTML = friendsArray.map(friend => `<span class="friend-box">${friend}</span>`).join('');
  }
}
